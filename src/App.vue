<template>
  <div id="search-app" class="search" :class="renderMode">
    ...
    <div id="search__result-wrapper" v-if="isShowingResultBlock" v-cloak>
      <div class="search__product-wrapper" v-show="showProductsBlock">
      ...
      <div class="search__result-folders-wrapper" v-show="showFoldersBlock">
      ...
      <div class="search__result-vendors-wrapper" v-show="showVendorsBlock">
...
export default {
  ...
  data() {
    return {
      ...
      fakeFolders:					           outerSettingsSmartSearch?.fakeFolders || [], // Фальшивые категории для поиска
      ...
      hiddenProducts:                 outerSettingsSmartSearch?.hidden_products || false,
      hiddenFolders:                  outerSettingsSmartSearch?.hidden_folders || false,
      hiddenVendors:                  outerSettingsSmartSearch?.hidden_vendors || false,
  ...
  mounted() {
    ...
    window.addEventListener("click", (e) =>
      !this.$el.contains(e.target)
        ? this.switchShowingResultBlockAfterRequest(false)
        : this.switchShowingResultBlockAfterRequest(true)
    );
    if (this.minimalAmountCharactersInSearch < 3) {
      this.minimalAmountCharactersInSearch = 3;
    }

    if (this.fakeFolders?.length) {
      this.foldersList = this.foldersList.concat(this.fakeFolders)
    }
  },
  ...
  methods: {
    startSearching() {
      if (this.isSearching) {
        this.typeRequest === "getSuggest" && !this.hiddenProducts ?  this.searchingProductsOnQuerySuggest()
            : this.typeRequest === "getProductsBySearchMatches" && !this.hiddenProducts? this.searchingProductsOnQueryMatches()
              : false;
      } else {
        this.resetData();
      }
     ...
     * @param flag
     */
    switchShowingResultBlockAfterRequest(flag) {
      this.$nextTick(() => {
    ...
    checkedOnWatchingResultBlock() {
      if ((this.showProductsBlock || this.showVendorsBlock || this.showFoldersBlock) && this.isSearching) {
        this.switchShowingResultBlockAfterRequest(true);
      } else {
        this.switchShowingResultBlockAfterRequest(false);
      }
  ...
  watch: {
     ...
     */
    showProductsBlock() {
      this.checkedOnWatchingResultBlock();
    },
  ...
  computed: {
    ...
    isShowingResultBlock() {
      return Boolean(
        this.canBeShownResultBlock &&
          (this.showProductsBlock ||
            this.showFoldersBlock ||
            this.showVendorsBlock)
      );
    },
    ...
    showProductsBlock() {
      return Boolean(this.foundProductsAfterApplyingLimits?.length) && !this.hiddenProducts;
    },
    ...
    showFoldersBlock() {
      return Boolean(this.foundFolders.length) && !this.hiddenFolders;
    },
    ...
    showVendorsBlock() {
      return Boolean(this.foundVendors.length) && !this.hiddenVendors;
    },
  },
};
</script>
